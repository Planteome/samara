...
class ScraperGrin$Test extends FlatSpec with Matchers {
  ...
  "scraper" should "be able to retrieve info for individual accessions" in {
    ...
    val expectedTaxon: GRINTaxon = GRINTaxon(name = "Triticum monococcum L. subsp. monococcum", id = 40597)
    ...
    actualAccession.accession.detail.references.filter(_.contains("Anker")) should not be empty
