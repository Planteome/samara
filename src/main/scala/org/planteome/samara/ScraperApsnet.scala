...
object ScraperApsnet extends Scraper with ResourceUtil with NameFinderStatic {
  ...
  override def scrape() = {
    println("disease_name\tsource_taxon_verbatim_name\tsource_taxon_name\tinteraction_type_label\tinteraction_type_id\ttarget_taxon_verbatim_name\ttarget_taxon_name\tsource_citation\tsource_url\tsource_accessed_at")
    scrapeDiseases().foreach {
      case (page: String, accessedAt: String, disease: Disease) => {
        ...
        println(s"${disease.name}\t${disease.verbatimPathogen}\t${disease.pathogen}\tpathogen of\thttp://purl.obolibrary.org/obo/RO_0002556\t${disease.verbatimHost}\t${disease.host}\t$citationFull\t$page\t$accessedAt")
      }
    }
